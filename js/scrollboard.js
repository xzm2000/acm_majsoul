        ...
        success: function(result) {
			...
			for (var i = result.result.length - 1 ; i >= 0; i--) {
				...
				if (sub.author.participantType=="VIRTUAL"){
					...
					subtime.setTime(tt.getTime()+sub.relativeTimeSeconds*1000);
					data.push(new Submit(sub.id, ss, sub.problem.index ,subtime , st));
					continue;
				}
    ...
    $.ajax({
        ...
        success: function(result) {
            for (var key in result.result.rows) {
				...
				if (team.participantType=="VIRTUAL"){
					continue;
					var ss=team.teamName;
					if (ss.indexOf("=")!=-1) 
					{
						ss=ss.substr(6,255);
					}
					while (ss!=ss.replace('.','_')) ss=ss.replace('.','_');
					while (ss!=ss.replace('*','_')) ss=ss.replace('*','_');
					data[ss] = new Team(ss, ss , null, false);
					continue;
				}
